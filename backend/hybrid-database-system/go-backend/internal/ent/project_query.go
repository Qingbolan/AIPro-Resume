...
func (pq *ProjectQuery) QueryDetail() *ProjectDetailQuery {
...
func (pq *ProjectQuery) Clone() *ProjectQuery {
	...
	return &ProjectQuery{
		...
		withDetail:       pq.withDetail.Clone(),
...
func (pq *ProjectQuery) WithDetail(opts ...func(*ProjectDetailQuery)) *ProjectQuery {
	query := (&ProjectDetailClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := pq.withDetail; query != nil {
		if err := pq.loadDetail(ctx, query, nodes, nil,
			func(n *Project, e *ProjectDetail) { n.Edges.Detail = e }); err != nil {
			return nil, err
		}
	}
...
func (pq *ProjectQuery) loadTechnologies(ctx context.Context, query *ProjectTechnologyQuery, nodes []*Project, init func(*Project), assign func(*Project, *ProjectTechnology)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_technologies" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (pq *ProjectQuery) loadDetail(ctx context.Context, query *ProjectDetailQuery, nodes []*Project, init func(*Project), assign func(*Project, *ProjectDetail)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.ProjectDetail(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.DetailColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.project_detail
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_detail" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_detail" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (pq *ProjectQuery) loadImages(ctx context.Context, query *ProjectImageQuery, nodes []*Project, init func(*Project), assign func(*Project, *ProjectImage)) error {
	...
	for _, n := range neighbors {
		fk := n.project_images
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_images" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_images" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
