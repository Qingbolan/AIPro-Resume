...
func (bcq *BlogCategoryQuery) loadBlogPosts(ctx context.Context, query *BlogPostQuery, nodes []*BlogCategory, init func(*BlogCategory), assign func(*BlogCategory, *BlogPost)) error {
	...
	for _, n := range neighbors {
		fk := n.blog_category_blog_posts
		if fk == nil {
			return fmt.Errorf(`foreign-key "blog_category_blog_posts" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "blog_category_blog_posts" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
