...
func (uq *UserQuery) QuerySocialLinks() *SocialLinkQuery {
	query := (&SocialLinkClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryBlogSeries() *BlogSeriesQuery {
	query := (&BlogSeriesClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withSocialLinks:      uq.withSocialLinks.Clone(),
		...
		withBlogSeries:       uq.withBlogSeries.Clone(),
...
func (uq *UserQuery) WithSocialLinks(opts ...func(*SocialLinkQuery)) *UserQuery {
	query := (&SocialLinkClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withSocialLinks = query
	return uq
}
...
func (uq *UserQuery) WithBlogSeries(opts ...func(*BlogSeriesQuery)) *UserQuery {
	query := (&BlogSeriesClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withBlogSeries = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withSocialLinks; query != nil {
		if err := uq.loadSocialLinks(ctx, query, nodes,
			func(n *User) { n.Edges.SocialLinks = []*SocialLink{} },
			func(n *User, e *SocialLink) { n.Edges.SocialLinks = append(n.Edges.SocialLinks, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := uq.withBlogSeries; query != nil {
		if err := uq.loadBlogSeries(ctx, query, nodes,
			func(n *User) { n.Edges.BlogSeries = []*BlogSeries{} },
			func(n *User, e *BlogSeries) { n.Edges.BlogSeries = append(n.Edges.BlogSeries, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadPersonalInfo(ctx context.Context, query *PersonalInfoQuery, nodes []*User, init func(*User), assign func(*User, *PersonalInfo)) error {
	...
	for _, n := range neighbors {
		fk := n.user_personal_info
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_personal_info" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_personal_info" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (uq *UserQuery) loadSocialLinks(ctx context.Context, query *SocialLinkQuery, nodes []*User, init func(*User), assign func(*User, *SocialLink)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.SocialLink(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.SocialLinksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_social_links
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_social_links" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_social_links" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadEducation(ctx context.Context, query *EducationQuery, nodes []*User, init func(*User), assign func(*User, *Education)) error {
	...
	for _, n := range neighbors {
		fk := n.user_education
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_education" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_education" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadWorkExperience(ctx context.Context, query *WorkExperienceQuery, nodes []*User, init func(*User), assign func(*User, *WorkExperience)) error {
	...
	for _, n := range neighbors {
		fk := n.user_work_experience
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_work_experience" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_work_experience" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadBlogSeries(ctx context.Context, query *BlogSeriesQuery, nodes []*User, init func(*User), assign func(*User, *BlogSeries)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.BlogSeries(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.BlogSeriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_blog_series
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_blog_series" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_blog_series" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadIdeas(ctx context.Context, query *IdeaQuery, nodes []*User, init func(*User), assign func(*User, *Idea)) error {
	...
	for _, n := range neighbors {
		fk := n.user_ideas
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_ideas" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_ideas" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadResearchProjects(ctx context.Context, query *ResearchProjectQuery, nodes []*User, init func(*User), assign func(*User, *ResearchProject)) error {
	...
	for _, n := range neighbors {
		fk := n.user_research_projects
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_research_projects" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_research_projects" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadPublications(ctx context.Context, query *PublicationQuery, nodes []*User, init func(*User), assign func(*User, *Publication)) error {
	...
	for _, n := range neighbors {
		fk := n.user_publications
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_publications" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_publications" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadAwards(ctx context.Context, query *AwardQuery, nodes []*User, init func(*User), assign func(*User, *Award)) error {
	...
	for _, n := range neighbors {
		fk := n.user_awards
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_awards" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_awards" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
