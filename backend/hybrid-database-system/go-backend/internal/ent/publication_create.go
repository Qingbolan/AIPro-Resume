...
func (pc *PublicationCreate) SetUserID(id uuid.UUID) *PublicationCreate {
	pc.mutation.SetUserID(id)
	return pc
}

// SetNillableUserID sets the "user" edge to the User entity by ID if the given value is not nil.
func (pc *PublicationCreate) SetNillableUserID(id *uuid.UUID) *PublicationCreate {
	if id != nil {
		pc = pc.SetUserID(*id)
	}
	return pc
...
func (pc *PublicationCreate) createSpec() (*Publication, *sqlgraph.CreateSpec) {
	...
	if nodes := pc.mutation.UserIDs(); len(nodes) > 0 {
		...
		_node.user_publications = &nodes[0]
