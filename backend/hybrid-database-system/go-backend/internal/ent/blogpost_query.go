...
func (bpq *BlogPostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*BlogPost, error) {
	...
	if bpq.withUser != nil || bpq.withCategory != nil || bpq.withSeries != nil {
		withFKs = true
	}
	if withFKs {
...
func (bpq *BlogPostQuery) loadCategory(ctx context.Context, query *BlogCategoryQuery, nodes []*BlogPost, init func(*BlogPost), assign func(*BlogPost, *BlogCategory)) error {
	...
	nodeids := make(map[uuid.UUID][]*BlogPost)
	for i := range nodes {
		if nodes[i].blog_category_blog_posts == nil {
			continue
		}
		fk := *nodes[i].blog_category_blog_posts
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "blog_category_blog_posts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (bpq *BlogPostQuery) loadSeries(ctx context.Context, query *BlogSeriesQuery, nodes []*BlogPost, init func(*BlogPost), assign func(*BlogPost, *BlogSeries)) error {
	...
	nodeids := make(map[uuid.UUID][]*BlogPost)
	for i := range nodes {
		if nodes[i].blog_series_blog_posts == nil {
			continue
		}
		fk := *nodes[i].blog_series_blog_posts
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "blog_series_blog_posts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
