...
type BlogPost struct {
	...
	blog_category_blog_posts *uuid.UUID
	blog_series_blog_posts   *uuid.UUID
	user_blog_posts          *uuid.UUID
...
func (*BlogPost) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case blogpost.ForeignKeys[1]: // blog_series_blog_posts
			values[i] = &sql.NullScanner{S: new(uuid.UUID)}
		case blogpost.ForeignKeys[2]: // user_blog_posts
			values[i] = &sql.NullScanner{S: new(uuid.UUID)}
		default:
	...
	for i := range columns {
		...
		case blogpost.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field blog_category_blog_posts", values[i])
			} else if value.Valid {
				bp.blog_category_blog_posts = new(uuid.UUID)
				*bp.blog_category_blog_posts = *value.S.(*uuid.UUID)
			}
		case blogpost.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field blog_series_blog_posts", values[i])
			} else if value.Valid {
				bp.blog_series_blog_posts = new(uuid.UUID)
				*bp.blog_series_blog_posts = *value.S.(*uuid.UUID)
			}
		case blogpost.ForeignKeys[2]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field user_blog_posts", values[i])
			} else if value.Valid {
				bp.user_blog_posts = new(uuid.UUID)
				*bp.user_blog_posts = *value.S.(*uuid.UUID)
			}
		default:
