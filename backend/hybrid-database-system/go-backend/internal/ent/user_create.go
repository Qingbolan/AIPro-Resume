...
func (uc *UserCreate) AddSocialLinkIDs(ids ...uuid.UUID) *UserCreate {
	uc.mutation.AddSocialLinkIDs(ids...)
	return uc
}

// AddSocialLinks adds the "social_links" edges to the SocialLink entity.
func (uc *UserCreate) AddSocialLinks(s ...*SocialLink) *UserCreate {
	ids := make([]uuid.UUID, len(s))
	for i := range s {
		ids[i] = s[i].ID
	}
	return uc.AddSocialLinkIDs(ids...)
}
...
func (uc *UserCreate) AddBlogSeriesIDs(ids ...uuid.UUID) *UserCreate {
	uc.mutation.AddBlogSeriesIDs(ids...)
	return uc
}

// AddBlogSeries adds the "blog_series" edges to the BlogSeries entity.
func (uc *UserCreate) AddBlogSeries(b ...*BlogSeries) *UserCreate {
	ids := make([]uuid.UUID, len(b))
	for i := range b {
		ids[i] = b[i].ID
	}
	return uc.AddBlogSeriesIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.SocialLinksIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
	...
	if nodes := uc.mutation.BlogSeriesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
